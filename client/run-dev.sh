...
echo -e "${CYAN_BOLD}=== ĐANG BẮT ĐẦU QUY TRÌNH PHÁT TRIỂN TANK-FOOD CLIENT ===${RESET}"
echo ""
...
echo -e "${BLUE_BOLD}[HÀNH ĐỘNG]${RESET} Đang khởi động TANK-Food Client..."
echo ""
npm run dev
if [ $? -ne 0 ]; then
    echo ""
    echo -e "${RED_BOLD}[LỖI]${RESET} Khởi động máy khách thất bại!"
    read -p "Nhấn Enter để thoát..."
    exit 1
fi
