...
export const getVoucherPrivateUnUsedController = async (
  ...
  try {
    const voucher = await voucherPrivateService.getVoucherUnUsed(user)
...
export const getVoucherPrivateUsedController = async (
  req: Request<ParamsDictionary, any, GetVoucherPrivateRequestsBody>,
  res: Response
) => {
  const ip = (req.headers['cf-connecting-ip'] || req.ip) as string
  ...
  try {
    const voucher = await voucherPrivateService.getVoucherUsed(user)
